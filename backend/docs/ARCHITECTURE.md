...
instance. A configured `API_TOKEN` protects writes; `TEARDOWN_TOKEN` separately
enables and protects reset (disabled without it). Health and metadata remain public. No rate limiter is enabled that
...
## Full-stack website (v2)

Netlify serves frontend HTML/CSS/ES modules. A single config.js supplies the Render
base URL, optionally overridden in one browser. Chat context/tick/reply requests
use `/demo/v1`, which runs the same engine over a separate SQLite store. `/v1`
remains the evaluator namespace. Public reads and CORS preflight work without
bearer auth; write auth is optional, shared configuration across both namespaces.
The UI does not synthesize successful replies when network calls fail. A failed
reply preserves its exact timestamp/turn/body for safe manual retry.
