...
The v2 Render Blueprint uses the Python runtime, manual deployment and a Free
demo instance, with no persistent disk. For durable SQLite, the full-stack guide
documents an explicitly selected paid service and attached disk. No cloud
deployment or Docker image build was executed. New frontend synthetic scenarios
are separately authored demo fixtures; the original challenge fixtures are retained.
