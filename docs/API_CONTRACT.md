...
Run from the project root with the server running. These files contain explicit
...
`POST /v1/teardown` clears all stored evaluation state. It is destructive and has
no confirmation dialog. Optional `TEARDOWN_TOKEN` overrides write auth for this
endpoint. Do not call during evaluation. Without tokens, this is a public reset.
