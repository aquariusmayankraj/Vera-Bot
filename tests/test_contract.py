...
def test_teardown_wipes_state(client, sample):
    load(client, sample)
    tick(client)
    assert client.post("/v1/teardown").json()["cleared"] is True
    assert sum(client.get("/v1/healthz").json()["contexts_loaded"].values()) == 0
    with client.app.state.store.transaction() as db:
        for table in ("contexts", "conversations", "recipients", "suppressions", "reply_cache"):
